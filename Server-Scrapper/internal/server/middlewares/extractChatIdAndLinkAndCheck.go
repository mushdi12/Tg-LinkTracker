...
func ExtractChatIdLinkAndCheck(storage *postgres.Storage) echo.MiddlewareFunc {
	op := "server.middlewares.ExtractChatIdLinkAndCheck"
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(ctx echo.Context) error {
			...
			if userLink == "" {
				return ctx.String(http.StatusBadRequest, "Некорректный link")
			}
